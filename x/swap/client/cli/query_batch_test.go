...
func TestListBatch(t *testing.T) {
	net, objs := networkWithBatchObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListBatch(), args)
			require.NoError(t, err)
			var resp types.QueryAllBatchResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
			require.LessOrEqual(t, len(resp.Batch), step)
			require.Subset(t,
            	nullify.Fill(objs),
            	nullify.Fill(resp.Batch),
            )
