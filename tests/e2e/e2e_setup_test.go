...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	vestingMnemonic := createMnemonic()
	jaiedMnemonic := createMnemonic()

	// The boostrapping phase is as follows:
	//
	// 1. Initialize validator nodes.
	// 2. Create and initialize validator genesis files (both chains)
	// 3. Start both networks.
	// 4. Create and run IBC relayer (Hermes) containers.

	s.T().Logf("starting e2e infastructure for chain A; chain-id: %s; datadir: %s", s.chainA.id, s.chainA.dataDir)
	s.initNodes(s.chainA)
	s.initGenesis(s.chainA, vestingMnemonic, jaiedMnemonic)
	s.initValidatorConfigs(s.chainA)
	s.runValidators(s.chainA, 0)

	s.T().Logf("starting e2e infastructure for chain B; chain-id: %s; datadir: %s", s.chainB.id, s.chainB.dataDir)
	s.initNodes(s.chainB)
	s.initGenesis(s.chainB, vestingMnemonic, jaiedMnemonic)
...
func (s *IntegrationTestSuite) initNodes(c *chain) {
	...
	*/
	...
	for _, addr := range c.genesisAccounts {
		address, err := addr.keyInfo.GetAddress()
		s.Require().NoError(err)
		addrAll = append(addrAll, address)
	}
	s.Require().NoError(
		modifyGenesis(c, val0ConfigDir, "", initBalanceStr, addrAll, initialGlobalFeeAmt+denom, denom),
	)
...
func (s *IntegrationTestSuite) addGenesisVestingAndJailedAccounts(
	...
	stakingModuleBalances := banktypes.Balance{
		...
		Coins:   sdk.NewCoins(sdk.NewCoin(denom, sdk.NewInt(slashingShares))),
	}
...
func (s *IntegrationTestSuite) initGenesis(c *chain, vestingMnemonic, jailedValMnemonic string) {
	...
	for i := range evidenceGenState.Evidence {
		...
		evidence := &evidencetypes.Equivocation{
			...
			Power:            100,
			Time:             time.Now().UTC(),
			ConsensusAddress: sdk.ConsAddress(pk.PubKey().Bytes()).String(),
		}
...
func (s *IntegrationTestSuite) initValidatorConfigs(c *chain) {
	for i, val := range c.validators {
		...
		appConfig.MinGasPrices = fmt.Sprintf("%s%s", minGasPrice, denom)
		...
		customAppTemplate := `
###############################################################################
###                    Custom GlobalFee Configuration                       ###
###############################################################################
