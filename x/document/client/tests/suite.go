...
func (s *DocumentIntegrationTestSuite) SetupSuite() {
	...
	out = tests.ExCmdEnrollIdSigner(
		s.network.Validators[0].ClientCtx,
		s.T(),
		s.network.Accounts[network.KeyAccount3].String(),
		network.SHRFee2(),
		network.MakeByAccount(network.KeyOperator),
		network.SkipConfirmation(),
	)
	...
	out = idtest.CmdExNewID(s.network.Validators[0].ClientCtx, s.T(), network.KeyAccount2,
		s.network.Accounts[network.KeyAccount2].String(),
		s.network.Accounts[network.KeyAccount2].String(),
		"hello",
	...
	out = idtest.CmdExGetID(s.network.Validators[0].ClientCtx, s.T(), network.KeyAccount2, network.JSONFlag())

	id := idtest.IDJsonUnmarshal(s.T(), out.Bytes())
	s.T().Logf("the id=%s", id.String())
...
func (s *DocumentIntegrationTestSuite) TestCreateDocument() {
	...
	s.Run("create_document_success_by_using_authority_account", func() {
		...
		out := CmdExCreateDocument(validationCtx,
			s.T(), network.KeyAccount2, proof, data,
		...
		s.Equalf(network.KeyAccount2, docResponse.GetHolder(), "holder ID isn't equal")
