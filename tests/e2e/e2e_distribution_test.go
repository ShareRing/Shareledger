...
func (s *IntegrationTestSuite) testDistribution() {
	...
	delegator, _ := s.chainA.genesisAccounts[2].keyInfo.GetAddress()
	delegatorAddress := delegator.String()

	newWithdrawal, _ := s.chainA.genesisAccounts[3].keyInfo.GetAddress()
	newWithdrawalAddress := newWithdrawal.String()
	fees := sdk.NewCoin(denom, sdk.NewInt(1000))

	beforeBalance, err := querySpecificBalance(chainEndpoint, newWithdrawalAddress, denom)
	s.Require().NoError(err)
	if beforeBalance.IsNil() {
		beforeBalance = sdk.NewCoin(denom, sdk.NewInt(0))
	}
	...
	s.Require().Eventually(
		func() bool {
			afterBalance, err := querySpecificBalance(chainEndpoint, newWithdrawalAddress, denom)
			s.Require().NoError(err)
...
func (s *IntegrationTestSuite) fundCommunityPool() {
	...
	beforeDistUatomBalance, _ := querySpecificBalance(chainAAPIEndpoint, distModuleAddress, tokenAmount.Denom)
	if beforeDistUatomBalance.IsNil() {
		// Set balance to 0 if previous balance does not exist
		beforeDistUatomBalance = sdk.NewInt64Coin(denom, 0)
	}
	...
	marginOfErrorForBlockReward := sdk.NewInt64Coin(denom, 500)

	s.Require().Eventually(
		func() bool {
			afterDistPhotonBalance, err := querySpecificBalance(chainAAPIEndpoint, distModuleAddress, tokenAmount.Denom)
