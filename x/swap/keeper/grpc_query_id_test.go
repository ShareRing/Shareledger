...
func TestIdQuerySingle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			response, err := keeper.Id(wctx, tc.request)
			if tc.err != nil {
...
func TestIdQueryPaginated(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.IdAll(wctx, request(nil, uint64(i), uint64(step), false))
			require.NoError(t, err)
	...
	t.Run("ByKey", func(t *testing.T) {
		...
		var next []byte
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.IdAll(wctx, request(next, 0, uint64(step), false))
			require.NoError(t, err)
			require.LessOrEqual(t, len(resp.Id), step)
			require.Subset(t,
				nullify.Fill(msgs),
	...
	t.Run("Total", func(t *testing.T) {
		resp, err := keeper.IdAll(wctx, request(nil, 0, 0, true))
		require.NoError(t, err)
