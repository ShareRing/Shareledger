...
Example:
	shareledger testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "shareledger", "Home directory of the node's daemon configuration")
...
func InitTestnet(
	...
	for i := 0; i < numValidators; i++ {
		...
		coins := sdk.Coins{
			...
			sdk.NewCoin(denom.Base, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(denom.Base, valTokens),
