...
func New(t *testing.T, cfg Config) *Network {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "shareledgerd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "shareledgercli")
	...
	accMap[KeyAuthority] = info.GetAddress()
	...
	info, _, err = network.Validators[0].ClientCtx.Keyring.NewMnemonic(KeyTreasurer, keyring.English, sdk.FullFundraiserPath, keyring.DefaultBIP39Passphrase, hd.Secp256k1)
	require.NoError(t, err, "init fail")
	accMap[KeyTreasurer] = info.GetAddress()
	genAccounts = append(genAccounts, authtypes.NewBaseAccount(info.GetAddress(), info.GetPubKey(), 0, 0))
	genBalances = append(genBalances, banktypes.Balance{
		Address: info.GetAddress().String(),
		Coins:   defaultCoins,
	...
	info, _, err = network.Validators[0].ClientCtx.Keyring.NewMnemonic(KeyOperator, keyring.English, sdk.FullFundraiserPath, keyring.DefaultBIP39Passphrase, hd.Secp256k1)
	require.NoError(t, err, "init fail")
	accMap[KeyOperator] = info.GetAddress()
	genAccounts = append(genAccounts, authtypes.NewBaseAccount(info.GetAddress(), info.GetPubKey(), 0, 0))
	genBalances = append(genBalances, banktypes.Balance{
		Address: info.GetAddress().String(),
		Coins:   defaultCoins,
	...
	info, _, err = network.Validators[0].ClientCtx.Keyring.NewMnemonic(KeyAccount1, keyring.English, sdk.FullFundraiserPath, keyring.DefaultBIP39Passphrase, hd.Secp256k1)
	require.NoError(t, err, "init fail")
	accMap[KeyAccount1] = info.GetAddress()
	genAccounts = append(genAccounts, authtypes.NewBaseAccount(info.GetAddress(), info.GetPubKey(), 0, 0))
	genBalances = append(genBalances, banktypes.Balance{
		Address: info.GetAddress().String(),
		Coins:   defaultCoins,
	...
	info, _, err = network.Validators[0].ClientCtx.Keyring.NewMnemonic(KeyAccount2, keyring.English, sdk.FullFundraiserPath, keyring.DefaultBIP39Passphrase, hd.Secp256k1)
	require.NoError(t, err, "init fail")
	accMap[KeyAccount2] = info.GetAddress()
	genAccounts = append(genAccounts, authtypes.NewBaseAccount(info.GetAddress(), info.GetPubKey(), 0, 0))
	genBalances = append(genBalances, banktypes.Balance{
		Address: info.GetAddress().String(),
		Coins:   defaultCoins,
	})
	info, _, err = network.Validators[0].ClientCtx.Keyring.NewMnemonic(KeyAccount3, keyring.English, sdk.FullFundraiserPath, keyring.DefaultBIP39Passphrase, hd.Secp256k1)
	require.NoError(t, err, "init fail")
	accMap[KeyAccount3] = info.GetAddress()
	genAccounts = append(genAccounts, authtypes.NewBaseAccount(info.GetAddress(), info.GetPubKey(), 0, 0))
	genBalances = append(genBalances, banktypes.Balance{
		Address: info.GetAddress().String(),
		Coins:   defaultCoins,
	...
	info, _, err = network.Validators[0].ClientCtx.Keyring.NewMnemonic(KeyAccount4, keyring.English, sdk.FullFundraiserPath, keyring.DefaultBIP39Passphrase, hd.Secp256k1)
	require.NoError(t, err, "init fail")
	accMap[KeyAccount4] = info.GetAddress()
	genAccounts = append(genAccounts, authtypes.NewBaseAccount(info.GetAddress(), info.GetPubKey(), 0, 0))
	genBalances = append(genBalances, banktypes.Balance{
		Address: info.GetAddress().String(),
		Coins:   defaultCoins,
...
func ShareLedgerTestingConfig() Config {
	encCfg := cosmoscmd.MakeEncodingConfig(app.ModuleBasics)
