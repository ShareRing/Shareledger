...
func (s *ElectoralIntegrationTestSuite) SetupSuite() {
	...
	out, _ := ExCmdEnrollAccountOperator(s.network.Validators[0].ClientCtx, s.T(), s.network.Accounts[network.KeyOperator].String(),
		network.SHRFee2(),
...
func (s *ElectoralIntegrationTestSuite) TestAccountOperator() {
	...
	s.Run("enroll_account_operator", func() {
		out, err := ExCmdEnrollAccountOperator(validatorCtx, s.T(), s.network.Accounts[network.KeyAccount1].String(),
			network.SHRFee2(),
			network.MakeByAccount(network.KeyAuthority),
			network.SkipConfirmation(),
			network.BlockBroadcast(),
	...
	s.Run("revoke_account_operator", func() {
		out, err := ExCmdRevokeAccountOperator(validatorCtx, s.T(), s.network.Accounts[network.KeyAccount1].String(),
			network.SHRFee2(),
			network.MakeByAccount(network.KeyAuthority),
			network.SkipConfirmation(),
			network.BlockBroadcast(),
	...
	s.Run("revoke_account_operator_but_not_authorizer", func() {
		out, err := ExCmdRevokeAccountOperator(validatorCtx, s.T(), s.network.Accounts[network.KeyOperator].String(),
			network.SHRFee2(),
...
func (s *ElectoralIntegrationTestSuite) TestDocIssuer() {
	...
	s.Run("enroll_doc_issuer", func() {
		out, err := ExCmdEnrollDocIssuer(validatorCtx, s.T(), s.network.Accounts[network.KeyDocIssuer].String(),
			network.SHRFee2(),
	...
	s.Run("revoke_doc_issuer_not_authorizer", func() {
		out, err := ExCmdRevokeDocIssuer(validatorCtx, s.T(), s.network.Accounts[network.KeyDocIssuer].String(),
			network.SHRFee2(),
...
func (s *ElectoralIntegrationTestSuite) TestIdSigner() {
	...
	s.Run("enroll_id_signer", func() {
		out, err := ExCmdEnrollIdSigner(validatorCtx, s.T(), s.network.Accounts[network.KeyIDSigner].String(),
			network.SHRFee2(),
			network.MakeByAccount(network.KeyOperator),
			network.SkipConfirmation(),
			network.BlockBroadcast(),
	...
	s.Run("revoke_id_signer_not_authorizer", func() {
		out, err := ExCmdRevokeIdSigner(validatorCtx, s.T(), s.network.Accounts[network.KeyIDSigner].String(),
			network.SHRFee2(),
			network.MakeByAccount(network.KeyAccount4),
			network.SkipConfirmation(),
			network.BlockBroadcast(),
