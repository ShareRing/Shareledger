...
func CmdApprove() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			signedHash, err := signApprovedSwap(clientCtx, signer, res.Request)
			if err != nil {
				return err
			}
...
func signApprovedSwap(ctx client.Context, signer string, requests []types.Request) (string, error) {
	...
	signerData.Message = apitypes.TypedDataMessage{
		"ids":     txIds,
		"tos":     destinations,
		"amounts": amounts,
	}
	signHash, err := crypto2.Keccak256HashEIP712(signerData)
	if err != nil {
		return "", err
	}
...
func tp(b []byte) tcrypto.Address {
