...
func (s *E2ETestSuite) CheckWithdrawReward(amount int64) {
	...
	s.Equal(reward.Sub(sdkmath.NewInt(amount)), after.Sub(before))
}

func (s *E2ETestSuite) CheckRewardNormalTx(amount int64) {
	val := s.network.Validators[0]
	...
	s.Equal(sdkmath.NewInt(amount), after.Sub(before))
}

func (s *E2ETestSuite) CheckRewardWasmTx(amount int64) {
	val := s.network.Validators[0]
	...
	after := s.getReward(devPoolAccount)
	// check acc1 account have reward of 0.4% * Fee = 0.4*10 = 4SHR
	s.Equal(sdkmath.NewInt(amount), afterAcc1.Sub(beforeAcc1))
	// check dev pool account have reward of 0.4% * Fee = 0.4*10 = 4SHR
	s.Equal(sdkmath.NewInt(amount), after.Sub(before))
}
